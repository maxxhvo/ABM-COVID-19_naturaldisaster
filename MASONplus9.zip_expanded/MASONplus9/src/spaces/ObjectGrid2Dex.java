...
public class ObjectGrid2Dex extends ObjectGrid2D {
	...
	public ObjectGrid2Dex (int width, int height)
	{
		super(width,height);
	}

	public ObjectGrid2Dex (int width, int height, Object initialValue)
	{
		super(width,height,initialValue);
	}

	public ObjectGrid2Dex (ObjectGrid2D values)
	{
		super(values);
	}

	public ObjectGrid2Dex(Object[][] values)
	{
		super(values);
	}
	...
	public boolean addAgentToLocation(Object o,final int x,final int y){
		if(field[x][y] != null)
			return false;
		else{
			field[x][y]=o;
	...
	public boolean removeAgentFromLocation(Object o,final int x,final int y){
		Object ox = field[x][y];
		if(ox == null)
			return false;
		else{
			if(o.equals(ox)){
				...
				return false; //object did not match object at location
			}
		}
	}
	
	public void setHardObjectLocation(Object o,final int x,final int y){
			field[x][y]=o;
	}

	public boolean setObjectLocation(Object o,final int x,final int y){
		if(field[x][y] != null)
			return false;
		else{
			field[x][y]=o;
	...
	public boolean setObjectLocation(Object o, Int2D location){
		return setObjectLocation(o,location.x,location.y);
	}

	public boolean setObjectLocation(Object o,final int ox,final int oy,final int x,final int y){
		if(field[x][y] != null)
			return false;
		else{
			field[x][y]=o;
	...
	public Int2D randomEmptyLocation(SimState state,final int x, final int y, final int mode, final int searchRadius, boolean includeOrigin){
		...
		for(int i = startx;i< xPos.numObjs;i++){
			Object b = field[xPos.objs[i]][yPos.objs[i]];
			if(b == null){
	...
	public Object getObjectAtLocation(final int x, final int y){
		return field[x][y];
	}
	...
	public void getMooreEmptyLocations( final int x, final int y, final int dist, int mode, boolean includeOrigin)
	{
		...
		if( toroidal )
		{
			...
			for( int x0 = xmin ; x0 <= xmax ; x0++ )
			{
				...
				for( int y0 = ymin ; y0 <= ymax ; y0++ )
				{
					...
					if(field[x_0][y_0] == null){ //only add null locations
						xPos.add( x_0 );
						yPos.add( y_0 );
					}
				}
		...
		else // not toroidal
		{
			...
			for( int x0 = xmin; x0 <= xmax ; x0++ )
			{
				for( int y0 = ymin ; y0 <= ymax ; y0++ )
				{
					if(field[x0][y0] == null){//add only null locations
						xPos.add( x0 );
						yPos.add( y0 );
					}
				}
