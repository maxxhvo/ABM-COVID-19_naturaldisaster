...
public class Agent implements Steppable {
	...
	public boolean infect(Environment state, Bag neighbors) { //need to define location
		if (this.isInHospital) return false;
	...
	public void colorByStatus(Environment state) {
	    switch(this.status) {
	        ...
	        case INFECTED:
	            // Set color to red
	            if (isInHospital)
	                // Dimmer red if the agent is in the hospital
	                state.gui.setOvalPortrayal2DColor(this, (float)1, (float)0, (float)0, (float)0.5);
	            else
	                state.gui.setOvalPortrayal2DColor(this, (float)1, (float)0, (float)0, (float)1);
	            break;
	        case RECOVERED:
	            // Set color to green
	...
	public Bag findNeighbors(Environment state) {
		if (this.isInHospital) return null; 
